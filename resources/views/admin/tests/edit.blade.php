            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-between items-center mb-6 border-b pb-4">
                    ...
                    <a href="{{ route('admin.tests.index') }}"
                        class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg transition duration-150">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2 mb-4">
                            ...
                            <input type="text" id="title" name="title" value="{{ old('title', $test->title) }}"
                                required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm"
                            ...
                            @foreach ($dataTypes as $value => $label)
                                <div class="flex items-center mb-2">
                                    @php
                                        ...
                                        $checkedValue = old(
                                            'required_data_type',
                                            $test->required_data_type ?? 'DATA_DIRI',
                                        );
                                    @endphp
                            ...
                            <select id="client_id" name="client_id"
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client->id }}"
                                        {{ old('client_id', $test->client_id) == $client->id ? 'selected' : '' }}>
                                        {{ $client->name }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select id="test_category_id" name="test_category_id" required
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('test_category_id', $test->test_category_id) == $category->id ? 'selected' : '' }}>
                            ...
                            <select id="jenjang_id" name="jenjang_id" required
                                ...
                                @foreach ($jenjangs as $jenjang)
                                    <option value="{{ $jenjang->id }}"
                                        {{ old('jenjang_id', $test->jenjang_id) == $jenjang->id ? 'selected' : '' }}>
                                        {{ $jenjang->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="duration_minutes" name="duration_minutes"
                                value="{{ old('duration_minutes', $test->duration_minutes) }}" required readonly
                        ...
                        <div>
                            ...
                            <input type="text" id="test_code" name="test_code"
                                value="{{ old('test_code', $test->test_code) }}" maxlength="8"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm">
                        ...
                        <div class="md:col-span-2 border border-gray-300 p-4 rounded-lg">
                            ...
                            <p class="mt-3 text-sm font-bold text-gray-700">Akumulasi Waktu Tes: <span
                                    id="total_duration_display">{{ old('duration_minutes', $test->duration_minutes) }}</span>
                                menit</p>
                            @error('alat_tes_ids')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="description" name="description" rows="3" required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm">{{ old('description', $test->description) }}</textarea>
                        </div>
                        ...
                        <div class="md:col-span-2 flex items-center space-x-6">
                            ...
                            <div class="flex items-center">
                                <input id="is_template" name="is_template" type="checkbox"
                                    {{ old('is_template', $test->is_template) ? 'checked' : '' }}
                                    class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
